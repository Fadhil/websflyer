defmodule WebsflyerWeb.AttributionWindowTest do
  ...
  describe "click event occurs when a user_attribution for that user_cookie doesn't exists" do
    test "a user_attribution is created with the user_cookie and affiliate name, attribution window and timestamp" do
      assert {:ok, media_source} =
               MediaSources.create_media_source(TestData.shopback_media_source())

      assert media_source.aff_name == "shopback"
      assert nil == UserAttributions.get_by_user_cookie("random1234usercookie")

      assert {:ok, _click_attribution} =
               Attributions.create_attribution(TestData.click_shopback_attrs())

      assert %UserAttribution{} =
               user_attribution = UserAttributions.get_by_user_cookie("random1234usercookie")
  ...
  describe "click event occurs when a user_attribution for that user_cookie exists" do
    test "the user_attribution is updated with the new affiliate attribution, attribution window and timestamp" do
      assert {:ok, _sb_media_source} =
               MediaSources.create_media_source(TestData.shopback_media_source())

      assert {:ok, _cs_media_source} =
               MediaSources.create_media_source(TestData.carousell_media_source())
      ...
      assert %UserAttribution{} =
               ...
               UserAttributions.get_by_user_cookie("random1234usercookie")
      ...
      assert {:ok, _click_attribution} =
               Attributions.create_attribution(TestData.click_carousell_attrs())

      assert %UserAttribution{} =
               second_user_attribution =
               UserAttributions.get_by_user_cookie("random1234usercookie")

      assert second_user_attribution.attributed_to == "carousell"
      assert first_user_attribution.id == second_user_attribution.id

      assert second_user_attribution.attribution_start_timestamp >
               first_user_attribution.attribution_start_timestamp

      assert second_user_attribution.attribution_window_in_seconds == 172_800
    end
  end
  ...
  describe "login event occurs" do
    test "when a user_attribution with the user_cookie exists, the user_attribution is updated with the user_id" do
      ...
      assert {:ok, _login_attribution} =
               Attributions.create_attribution(TestData.login_user_1234_attrs())
  ...
  describe "transaction event occurs" do
    test "creates transaction attribution when a user click_attribution and login_attribution exists and the transaction occurs within the user_attribution's attribution window" do
      ...
      assert {:ok, transaction_attribution} =
               Attributions.create_attribution(TestData.transaction_user_1234_attrs())
    ...
    test "doesn't create a transaction attribution when  the transaction occurs outside of the user_attribution's attribution window" do
      ...
      {:ok, _updated_ua} =
        UserAttributions.update_user_attribution(user_attribution, %{
          attribution_start_timestamp: user_attribution.attribution_start_timestamp - 90000
        })
