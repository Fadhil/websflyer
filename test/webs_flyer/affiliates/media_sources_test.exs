defmodule WebsFlyer.Affiliates.MediaSources.MediaSourcesTest do
  ...
  describe "media_sources" do
    ...
    test "create_media_source/1 with valid data creates a media_source" do
      assert {:ok, %MediaSource{} = media_source} = Affiliates.MediaSources.create_media_source(@valid_ms_attrs)
    ...
    test "create_media_source/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Affiliates.MediaSources.create_media_source(@invalid_ms_attrs)
    end

    test "update_media_source/2 with valid data updates the media_source" do
      media_source = media_source_fixture()
      assert {:ok, media_source} = Affiliates.MediaSources.update_media_source(media_source, @update_attrs)
    ...
    test "update_media_source/2 with invalid data returns error changeset" do
      media_source = media_source_fixture()
      assert {:error, %Ecto.Changeset{}} = Affiliates.MediaSources.update_media_source(media_source, @invalid_ms_attrs)
    ...
    test "delete_media_source/1 deletes the media_source" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Affiliates.MediaSources.get_media_source!(media_source.id) end
    end
  ...
  describe "get_attribution_window/1" do
    ...
    test "returns a default attribution of 30 days if media_source with given aff_name does not exist" do
      assert 2592000 = Affiliates.MediaSources.get_attribution_window("non_existent_affiliate")
    end

    test "returns a default attribution window of 30 days hours if media_source doesn't have attribution_window_in_seconds" do
      _media_source = Affiliates.MediaSources.create_media_source(@valid_ms_attrs_without_attribution_window)
      assert 2592000 = Affiliates.MediaSources.get_attribution_window("affiliate_no_attribution")
    end
  end
end