defmodule WebsflyerWeb.TrackingController do
  ...
  defp set_tracking_code(conn) do
    ...
    if is_nil(user_cookie) do
      put_resp_cookie(
        conn,
        "_websflyer_id",
        Phoenix.Token.sign(conn, "affiliate salt", System.system_time())
      )
    else
      conn
    end
  ...
  defp update_attribution(conn) do
    ...
    conn =
      case {user_cookie, user_id, url_params, rs_id} do
        ...
        {user_cookie, user_id, url_params, _rs_id}
        when not is_nil(user_cookie) and not is_nil(url_params) and not is_nil(user_id) ->
        ...
        {user_cookie, _user_id, url_params, _rs_id}
        when not is_nil(user_cookie) and not is_nil(url_params) ->
          Attributions.create_attribution(%{
            "user_cookie" => user_cookie,
            "url_params" => url_params,
            "event" => "click"
  ...
  defp should_track?(conn) do
    ...
    utm_source not in ["", nil] ||
      (user_id not in ["", nil] &&
         (is_nil(user_ident_cookie) || is_changed?(user_ident_cookie, user_id)))
  end
