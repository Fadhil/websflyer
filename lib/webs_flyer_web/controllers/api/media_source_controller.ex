defmodule WebsFlyerWeb.API.MediaSourceController do
  ...
  def create(conn, %{"media_source" => media_source_params}) do
    with {:ok, %MediaSource{} = media_source} <- MediaSources.create_media_source(media_source_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "media_source" => media_source_params}) do
    ...
    with {:ok, %MediaSource{} = media_source} <- MediaSources.update_media_source(media_source, media_source_params) do
      render(conn, "show.json", media_source: media_source)
    end
  end
